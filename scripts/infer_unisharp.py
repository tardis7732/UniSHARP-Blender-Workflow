...
def _force_square_pixel_intrinsics(intrinsics: torch.Tensor) -> torch.Tensor:
    """Keep the horizontal calibration and make the image pixels square."""
    ...
    *,
    ...
    for reload_attempt in range(2):
        ...
        if explicit_intrinsics is not None:
            ...
            if camera_kind == "perspective" and args.force_square_pixels:
                render_intrinsics = _force_square_pixel_intrinsics(render_intrinsics)
    ...
    # The Blender workflow only needs the original Gaussian PLY. Avoid
    # rendering the forward/turntable PNG sequences and GIFs in that case.
    if args.blender_only:
        ...
        _save_ply_if_requested(
            gaussians_world,
            sample_dir / "gaussians.ply",
            f_px=f_px,
            image_h=h,
            image_w=w,
            enabled=True,
            intrinsics=render_intrinsics if camera_kind == "perspective" else None,
        )
        LOGGER.info("Saved Blender-workflow PLY -> %s", sample_dir / "gaussians.ply")
        return
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--blender-only",
        action="store_true",
        help="Save only the Gaussian PLY needed for Blender; skip rendered frames, GIFs, and metadata.",
    )
    p.add_argument(
        "--camera-json",
        type=Path,
    ...
    p.add_argument(
        "--force-square-pixels",
        action="store_true",
        help="For perspective images, keep fitted fx and force fy=fx before Gaussian inference. Useful when a normal photo's vertical overlay is compressed.",
    )
    p.add_argument(
        "--camera-params",
        type=float,
